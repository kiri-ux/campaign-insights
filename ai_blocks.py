...
def _classify_batch(rows, kind, api_key, model):
    ...
    prompt = (
        ...
        f"- NEWS of any kind — publishers, local TV/radio news sites and apps, "
        f"tabloids, AND news aggregator/reader apps (Opera News, SmartNews, "
        f"NewsBreak, Sliide, etc.). Aggregating news is a legitimate format, not "
        f"a content farm. The ONLY news-adjacent thing to flag is an outright "
        f"FAKE-news domain fabricated to look like news (typo-squats, .store/.online "
        f"TLD abuse, no real newsroom) — categorize those 'Misinformation'\n"
        f"- weather, sports, or local business/organization sites\n"
    ...
    for v in verdicts:
        try:
            ...
            out.append({"name": r["name"], "app_id": r.get("app_id", r["name"]),
                        ...
                        "raw_value": str(r.get("raw_value", "") or ""),
